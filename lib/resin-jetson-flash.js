...
module.exports = class ResinJetsonFlash {
	constructor(deviceType, image, odmdata, output) {
		...
		this.dictionary = {
			...
			['jetson-agx-orin-auvidea-x230d']: {
				valid_target: 
					'jetson-agx-orin-devkit',
				url:
					'./images/Linux_for_Tegra_R35.3.1_auvdiea_x230d.tar.bz2',
				partitions: {
					'resin-boot': { path: null },
					'resin-rootA': { path: null },
					'resin-rootB': { path: null },
			...
			['jetson-agx-orin-aetina-mx11']: {
				valid_target: 
					'jetson-agx-orin-devkit',
				url:
					'./images/Linux_for_Tegra_R35.3.1_aetina_mx11.tar.bz2',
				partitions: {
					'resin-boot': { path: null },
					'resin-rootA': { path: null },
					'resin-rootB': { path: null },
		...
		this.customTargets = ['jetson-agx-orin-auvidea-x230d','jetson-agx-orin-aetina-mx11'];
		if (this.customTargets.includes(this.deviceType))
		{
			this.assetDir = `${__dirname}/../assets/${this.dictionary[deviceType].valid_target}-assets`;
		}
		else{
			this.assetDir = `${__dirname}/../assets/${this.deviceType}-assets`;
		}
	...
	spawnFlash() {
		return new Bluebird(async (resolve, reject) => {
			...
			if (this.deviceType.toString().startsWith("jetson-agx-orin" )) {
			...
			if ("default" != this.odmdata.toString()) {
				if ( this.deviceType.toString().startsWith("jetson-tx2") )
					{
			...
			const child = spawn(
				...
				[
					...
					((this.customTargets.includes(this.deviceType)) ? this.dictionary[this.deviceType].valid_target :  this.deviceType),
					'mmcblk0p12',
				],
				{
					stdio: 'inherit',
	...
	async generateArtifacts() {
		...
		if (!(await utils.checkConsistency(nvidiaToolPath))) {
			if (this.customTargets.includes(this.deviceType.toString())) {
				console.log("Custom target detected")
				await utils.decompressTbz2FromFile(
